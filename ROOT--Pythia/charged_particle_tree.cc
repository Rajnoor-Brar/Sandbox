...
int main() {
  ...
  pythia.readString("Beams:eCM = "+sBeam);
  
  pythia.readString("HeavyIon:mode = 1");       // Enable heavy-ion collisions

  pythia.readString("HadronLevel:BoseEinstein = on");

  Bool_t kQCDCR = kFALSE;
  Bool_t kRopes = kFALSE;
  Bool_t kShoving = kFALSE;
  ...
  sEv = to_string(nEvents);
  TString sE = "E";
  sEv = sEv(0)+ sE +to_string(sEv.Length()-1);
  sTime = to_string(T->GetTime());
  string sPyt = "Random:seed ="+to_string(T->GetTime());
  pythia.readString("Random:setSeed = on");
  pythia.readString(sPyt);
  ...
  TFile* outFile = new TFile(sPath+sEv+"_"+sBeam+"_"+sTime+".root", "RECREATE");

  TH1F* hdNdeta = new TH1F("hdNdeta", "dN_{ch}/d#eta; #eta; 1/N_{events} dN_{ch}/d#eta", 60, -ymax, ymax);
  ...
  for (int iEvent = 0; iEvent < nEvents; ++iEvent) {
    ...
    for (int i = 0; i < pythia.event.size(); ++i) {
      ...
      if(!kPartons){
        ...
        if(pythia.event[i].id()== 333 || pythia.event[i].id()== 313)
          kResonance=kTRUE;
      ...
      track->SetMomentum(pythia.event[i].px(), pythia.event[i].py(), pythia.event[i].pz(), pythia.event[i].e());
